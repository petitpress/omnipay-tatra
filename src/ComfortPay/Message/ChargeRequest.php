...
class ChargeRequest extends AbstractSoapRequest
{
        ...
        try {
            ...
            if ($sf->faultcode === 'SOAP-ENV:Receiver') {
                return $this->response = new CardTransactionResponse($this, [
                    ...
                    'transactionStatus' => $this->getSoapFaultErrorCode($sf->detail),
    ...
    private function getSoapFaultErrorCode($detail)
    {
        if (!$detail) {
            return null;
        }

        if (isset($detail->ExceptionType->errorCode)) {
            $errorCode =  (int) $detail->ExceptionType->errorCode;
            if ($errorCode >= 50000) {
                return $errorCode;
            }
        }
